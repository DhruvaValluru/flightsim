...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    # Counted the way the verifier counts: NOT RUN is not a failure, and
    # it is NOT a pass either. Reporting "25/25" for a run where ten
    # checks had no independent reference to compare against is exactly
    # the over-claiming this phase's verification is supposed to end.
    from core.capture.verify import FAIL, NOT_RUN, PASS

    checks = list(first.checks) + list(second.checks) + [alignment]
    passed = sum(c.status == PASS for c in checks)
    failed = sum(c.status == FAIL for c in checks)
    skipped = [c.name for c in checks if c.status == NOT_RUN]
    ok = first.ok and second.ok and alignment.ok
    print()
    print(f"DEMO {'PASSED' if ok else 'FAILED'} across two runs: "
          f"{passed} passed, {failed} failed, {len(skipped)} not run")
    if skipped:
        print(f"  not run (no independent reference on this platform; "
              f"NOT counted as passes): "
