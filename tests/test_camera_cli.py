...
def test_the_documents_windows_frames_block_matches_the_stub_run(tmp_path):
    """The Windows section's expected output (section 2 of "Engine
    verification (Windows)") is the stdout of the --render frames run
    of cameras_multi on the honest engine stub, generated by
    scripts/examples_expected.py, with the engine_parity row's
    MEASURED digits masked x. A fresh stub run here must reproduce it
    the way the Linux blocks are reproduced (exact on the measured
    platform, numbers masked elsewhere); the block must carry the
    header, the JSBSim line, both schedule lines, both engine passes,
    the overlays, the clip, the ten-row table and the "done:" verdict;
    and the section's typed tree (verify.json's counts, run.json's
    keys) must agree with the files the run wrote."""
    module = _examples_expected_module()
    doc_text = module.DOC.read_text(encoding="utf-8")
    doc = module.frames_doc_block(doc_text)
    assert doc is not None, "the frames_expected markers or block are missing"
    exact = module.measured_platform(doc_text) == module.this_platform()
    fresh = module.generate_frames(tmp_path / "runs_root")
    assert fresh["code"] == 0, fresh["stderr"]
    assert doc["command"] == fresh["command"] == module.FRAMES_COMMAND[1]
    assert doc["code"] == fresh["code"] == 0
    ...
    text = doc["text"]
    # The shape of the Windows command's stdout, not a summary of it.
    for line in ("spec cef57d752362381d valid; running headlessly...",
                 "run:         runs/demo", "cameras      2",
                 "  card:     runs/demo/card.json (consume-poses; one "
                 "commandlet pass per camera via -camera-index=N)",
                 "JSBSim output: runs/demo/jsbsim.log (15 model loads; "
                 "nothing of JSBSim's on stdout)",
                 "scheduled 48 frames across 2 camera(s)",
                 "  chase0: 24 scheduled instant(s)",
                 "  tower0: 24 scheduled instant(s)",
                 "engine pass 1 of 2: camera 'chase0', 24 frames scheduled "
                 "over the 12 s run (-camera-index=0)",
                 "engine pass 2 of 2: camera 'tower0', 24 frames scheduled "
                 "over the 12 s run (-camera-index=1)",
                 "  overlays: 48 reprojected-geometry overlay(s)",
                 "  clip:     runs/demo/clip.mp4 (by-product of camera "
                 "'chase0', 24 frames at their scheduled instants; 12.992 s",
                 "verification PASSED (10/10 checks)",
                 "done: rendered 48 frames across 2 camera(s) (48 verified "
                 "by engine parity) under runs/demo/frames"):
        assert line in text, line
    assert text.rstrip("\n").splitlines()[-1].startswith("done: ")
    assert "Mass Properties" not in text and "\x1b[" not in text
    # Only the engine's row is masked, and it is masked: no digit in its
    # MEASURED cell, every digit of the other rows kept.
    rows = verification_rows(text)
    assert rows["engine_parity"][1] == "pos x.xxx m, ang x.xxx deg, t x.xe+xx s, px x.xx"
    assert rows["engine_parity"][2] == "0.1 m, 0.1 deg, 1e-06 s, 3.0 px"
    assert rows["geometry_recovery"][1] == "4.1e-13 px" if exact else "px" in rows["geometry_recovery"][1]
    assert len(rows) == 10
    assert all(status == "PASS" for status, *_ in rows.values())
    # The typed tree of section 4 states what verify.json and run.json
    # hold; the files the run wrote are the authority.
    verify = json.loads((fresh["run_dir"] / "verify.json").read_text(encoding="utf-8"))
    tree = re.search(r"^  verify\.json +.*?checks \[(\d+), .*?passed (\d+), "
                     r"ran (\d+), awaiting \[\]", doc_text, re.M)
    assert tree, "section 4's verify.json line is missing or reworded"
    assert [int(g) for g in tree.groups()] == [
        len(verify["checks"]), verify["passed"], verify["ran"]]
    assert verify["awaiting"] == [] and verify["ok"] is True
    run = json.loads((fresh["run_dir"] / "run.json").read_text(encoding="utf-8"))
    run_line = re.search(r"^  run\.json +(.*)$", doc_text, re.M)
    assert run_line, "section 4's run.json line is missing"
    for key in run:
